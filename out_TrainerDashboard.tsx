    ...
    <div className="animate-fade-in space-y-5">
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-muted text-sm mt-1">{exercises.length} ejercicios</p>
        </div>
        <Button className="gap-2" onClick={() => setShowAdd(true)}>
        ...
        {filtered.map(({ name, idx }) => (
          <div key={idx} className="flex items-center gap-3 px-4 py-3 hover:bg-bg-alt/40 transition-colors group">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              {editingIdx === idx ? (
                <>
                  <button onClick={confirmEdit} className="p-1.5 rounded-lg bg-ok/10 text-ok hover:bg-ok/20 transition-colors" title="Confirmar"><Check className="w-4 h-4" /></button>
                  <button onClick={() => setEditingIdx(null)} className="p-1.5 rounded-lg text-muted hover:bg-bg transition-colors" title="Cancelar"><X className="w-4 h-4" /></button>
                </>
              ) : deletingIdx === idx ? (
                <>
                  <button onClick={() => del(idx)} className="px-2 py-1 bg-warn/10 text-warn border border-warn/20 rounded text-[10px] font-bold uppercase">Borrar</button>
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingIdx(idx); setEditVal(exercises[idx]) }} className="p-1.5 rounded-lg text-muted hover:text-accent hover:bg-accent/10 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                  <button onClick={() => setDeletingIdx(idx)} className="p-1.5 rounded-lg text-muted hover:text-warn hover:bg-warn/10 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              )}
            </div>
          </div>
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setNewName('') }} title="Nuevo ejercicio">
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button className="flex-1 gap-2" onClick={add} disabled={!newName.trim()}>
    ...
    <div className="animate-fade-in space-y-5">
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-muted text-sm mt-1">Crea y reutiliza tus programas</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map(t => (
          <div key={t.id} className="p-5 bg-card border border-border rounded-2xl hover:border-accent/40 transition-all group">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-1">
                ...
                {deletingId === t.id ? (
                  <>
                    <button onClick={() => del(t.id)} className="px-2 py-1 bg-warn/10 text-warn border border-warn/20 rounded text-[10px] font-bold uppercase">Borrar</button>
        ...
        {editing && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-muted mb-1.5">Nombre *</label>
              <input
                autoFocus type="text" placeholder="Ej: Fuerza Intermedio 8 semanas"
                className="w-full px-4 py-3 bg-bg border border-border rounded-lg text-sm outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all"
                value={editing.name}
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1 gap-2" onClick={save} disabled={!editing.name.trim()}>
