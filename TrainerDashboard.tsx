    ...
    <div className="flex h-screen overflow-hidden bg-bg">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-8 py-8">
          ...
          {/* ── PLANTILLAS ── */}
          {activeTab === 'templates' && <TemplatesTab clients={clients} onSelectClient={onSelectClient} />}
...
function ExercisesTab() {
  ...
  const confirmEdit = () => {
    ...
    if (!trimmed) return
    setExercises(prev => { const u = [...prev]; u[editingIdx] = trimmed; return u.sort() })
    setEditingIdx(null)
    toast('Ejercicio editado ✓', 'ok')
  }
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="bg-card border border-border rounded-2xl divide-y divide-border overflow-hidden">
        {filtered.length === 0 && (
          <div className="p-8 text-center text-muted text-sm">No se encontró "{search}"</div>
        )}
        {filtered.map(({ name, idx }) => (
          <div key={idx} className="px-4 py-3 flex items-center gap-3 hover:bg-bg-alt/50 transition-colors group">
            ...
            {editingIdx === idx ? (
              <input
                ...
                className="flex-1 bg-bg border border-accent/40 rounded-lg px-3 py-1.5 text-sm font-medium outline-none focus:ring-2 focus:ring-accent/20"
              />
            ) : (
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              {editingIdx === idx ? (
                ...
                <>
                  ...
                  <button onClick={() => setDeletingIdx(null)} className="px-2 py-1 bg-bg border border-border rounded text-[10px] font-bold uppercase text-muted">No</button>
                </>
              ) : (
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => { setEditingIdx(idx); setEditingValue(exercises[idx]) }}
...
// ══════════════════════════════════════════════════════════
interface Template {
  id: string; name: string; type: string; weeks: number; description: string
}
...
const DEFAULT_TEMPLATES: Template[] = [
  ...
  { id: '3', name: 'Pérdida de grasa — Avanzado', type: 'perdida_grasa', weeks: 12, description: 'Alta frecuencia con déficit calórico. Combinación de fuerza y metabólico.' },
]

function TemplatesTab({ clients, onSelectClient }: { clients: ClientData[]; onSelectClient: (c: ClientData) => void }) {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map(t => (
          <div key={t.id} className="p-5 bg-card border border-border rounded-2xl hover:border-accent/50 transition-all group">
            <div className="flex items-start justify-between mb-3">
              <div className="w-10 h-10 rounded-xl bg-bg flex items-center justify-center text-muted flex-shrink-0">
                <ClipboardList className="w-5 h-5" />
              </div>
              <div className="flex gap-1">
                <button onClick={() => openEdit(t)} className="p-1.5 rounded-lg text-muted hover:text-accent hover:bg-accent/10 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                {deletingId === t.id ? (
                  <>
                    ...
                    <button onClick={() => setDeletingId(null)} className="px-2 py-1 bg-bg border border-border rounded text-[10px] font-bold uppercase text-muted">No</button>
                  </>
                ) : (
                  <button onClick={() => setDeletingId(t.id)} className="p-1.5 rounded-lg text-muted hover:text-warn hover:bg-warn/10 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                )}
              </div>
            </div>
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {TRAINING_TYPES.map(tt => (
                  <button
                    key={tt.value}
                    type="button"
                    onClick={() => setEditing(prev => prev ? { ...prev, type: tt.value } : prev)}
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-bg border border-border rounded-lg outline-none focus:ring-2 focus:ring-accent/20 text-sm"
                value={editing.weeks}
                onChange={e => setEditing(prev => prev ? { ...prev, weeks: parseInt(e.target.value) || 1 } : prev)}
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 bg-bg border border-border rounded-lg outline-none focus:ring-2 focus:ring-accent/20 text-sm resize-none"
