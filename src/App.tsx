...
const TrainerDashboard = lazy(() =>
  import("./components/trainer/TrainerDashboard").then((m) => ({
    default: m.TrainerDashboard,
  })),
);
const ClientPanel = lazy(() =>
  import("./components/trainer/ClientPanel").then((m) => ({
    default: m.ClientPanel,
  })),
);
const ClientView = lazy(() =>
  import("./components/client/ClientView").then((m) => ({
    default: m.ClientView,
  })),
);
const SuperAdminPanel = lazy(() =>
  import("./components/trainer/SuperAdminPanel").then((m) => ({
      ...
      <div className="max-w-lg w-full bg-card border border-border rounded-3xl p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted">
          ...
          <span className="font-semibold text-ink">
            {" "}
            Tus clientes y datos siguen guardados
          </span>{" "}
          y se recuperan al instante al reactivar.
        </p>
        <p className="text-xs text-muted">
          Contacta a soporte/comercial para activar tu plan y continuar donde lo
          dejaste.
        </p>
        <button
          onClick={onLogout}
          className="px-4 py-2.5 rounded-xl bg-ink text-white text-sm font-semibold hover:opacity-90"
        >
          Cerrar sesión
        </button>
      </div>
...
export default function App() {
  ...
  const loadProfile = async (uid: string, email: string) => {
    ...
    const trialExpired = !!(
      profile.planName === "trial" &&
      profile.trialEndsAt &&
      Date.now() > profile.trialEndsAt
    );
    if (trialExpired) {
      setUserProfile({
        uid,
  ...
  if (view === "demo") {
    localStorage.setItem(
      `pf_trainer_profile_${DEMO_TRAINER_ID}`,
      JSON.stringify(DEMO_TRAINER_PROFILE),
    );
    localStorage.setItem(
      `pf_trainer_phone_${DEMO_TRAINER_ID}`,
      DEMO_TRAINER_PROFILE.phone,
    );
    // Pesos corporales

    localStorage.setItem(
      `pf_weight_demo-client-001`,
      JSON.stringify(DEMO_WEIGHTS_MARIA),
    );
    localStorage.setItem(
      `pf_weight_demo-client-002`,
      JSON.stringify(DEMO_WEIGHTS_CARLOS),
    );
    localStorage.setItem(
      `pf_weight_demo-client-003`,
      JSON.stringify(DEMO_WEIGHTS_LAURA),
    );
    // Encuestas demo

    localStorage.setItem(
      `pf_demo_survey_template`,
      JSON.stringify(DEMO_SURVEY_TEMPLATE),
    );
    localStorage.setItem(
      `pf_demo_survey_responses`,
      JSON.stringify(DEMO_SURVEY_RESPONSES),
    );
  }
  ...
  const encuestaParam =
    new URLSearchParams(window.location.search).get("encuesta") === "1";

  if (view === "client-token" && clientToken)
    return (
  ...
  if (view === "trial-expired" && userProfile?.role === "trainer")
    return (
      <>
      ...
      <Suspense fallback={<LoadingScreen />}>
        {selectedClient ? (
          <ClientPanel
            ...
            onClose={() => setSelectedClient(null)}
            demoPlan={
              selectedClient.id === "demo-client-001"
                ? DEMO_PLAN_MARIA
                : selectedClient.id === "demo-client-002"
                  ? DEMO_PLAN_CARLOS
                  : DEMO_PLAN_LAURA
            }
            demoLogs={
