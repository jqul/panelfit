...
import { ChevronDown, ChevronUp, Play, Check, Dumbbell } from 'lucide-react'
          ...
          return (
            <div key={di} className={`bg-card border rounded-2xl overflow-hidden transition-all ${isComplete ? 'border-ok/40' : 'border-border'}`}>
              <div className="flex items-center gap-3 px-4 py-4">
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setOpenDays(p => ({ ...p, [dayKey]: !p[dayKey] }))}>
                  ...
                  {day.exercises.map((ex, ri) => {
                    ...
                    return (
                      <div key={ri} className={`flex items-center gap-3 px-4 py-3 ${log?.done ? 'opacity-60' : ''}`}>
                        ...
                        {ytId && (
                          <a href={ex.videoUrl} target="_blank" rel="noreferrer" className="w-12 h-8 rounded-lg overflow-hidden flex-shrink-0 border border-border">
