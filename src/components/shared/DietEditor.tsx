    ...
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-bg-alt/50" onClick={() => setOpen(o => !o)}>
        ...
        <div className="flex-1 grid grid-cols-3 gap-3 min-w-0">
          ...
          <input type="text" placeholder="Nombre (ej: Desayuno)" value={meal.name} onClick={e => e.stopPropagation()} onChange={e => onChange({ ...meal, name: e.target.value })}
            className="bg-transparent text-sm font-semibold outline-none border-b border-transparent focus:border-accent" />
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={e => { e.stopPropagation(); onDelete() }} className="p-1.5 rounded-lg text-muted hover:text-warn transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
        ...
        <div className="px-4 pb-4 space-y-2 border-t border-border">
          ...
          {meal.items.map((item, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <input type="text" placeholder="Ej: Avena 60g, Leche 200ml..." value={item} onChange={e => updateItem(i, e.target.value)}
                ...
                <div key={m.label} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      ...
                      <p className="text-xs text-muted">{Math.round(diet.kcal * m.pct / 100)} kcal</p>
                    </div>
            ...
            <div key={meal.id} className="bg-card border border-border rounded-2xl p-4">
              ...
              <ul className="space-y-1">
                {meal.items.filter(Boolean).map((item, j) => (
                  <li key={j} className="flex items-start gap-2 text-sm text-muted"><span className="text-accent mt-0.5 flex-shrink-0">·</span>{item}</li>
                ))}
              </ul>
            </div>
          ))}
      ...
      <div className="space-y-3">
        ...
        {diet.meals.length === 0 && (
          <div className="bg-card border-2 border-dashed border-border rounded-2xl p-10 text-center text-muted">
            ...
            <p className="text-sm">Aún no has añadido comidas.</p>
          </div>
        )}
        {sortedMeals.map((meal, i) => (
