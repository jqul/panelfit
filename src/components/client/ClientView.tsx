  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <main className="flex-1 pb-20 max-w-2xl mx-auto w-full">
        {activeTab === 'hoy' && (
          plan
            ? <ClientDashboard plan={plan} logs={logs} onLogsChange={handleLogsChange} weightHistory={weightHistory} clientName={clientName} />
            : <div className="p-6 text-center text-muted py-20"><p className="font-serif text-lg">Sin plan asignado</p><p className="text-sm mt-1">Tu entrenador aún no ha creado tu plan.</p></div>
        )}

        {activeTab === 'entreno' && (
          plan
            ? <TrainingPlanView plan={plan} logs={logs} onLogsChange={handleLogsChange} />
            : <div className="p-6 text-center text-muted py-20"><Dumbbell className="w-10 h-10 mx-auto mb-3 opacity-30" /><p className="font-serif text-lg">Sin plan de entrenamiento</p></div>
        )}
        ...
        {activeTab === 'dieta' && <DietClientView clientId={client?.id || ''} />}
        ...
        <div className="flex max-w-2xl mx-auto">
          {TABS.map(({ id, icon: Icon, label }) => (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`flex-1 flex flex-col items-center justify-center gap-1 py-3 transition-colors ${activeTab === id ? 'text-ink' : 'text-muted'}`}
            >
...
// ── Dieta del cliente (solo lectura) ─────────────────────
