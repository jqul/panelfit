...
function SetRow({ setNum, weight, reps, done, prevWeight, prevReps, isMain, onWeightChange, onRepsChange, onToggle }: {
  setNum: number; weight: string; reps: string; done: boolean
  prevWeight?: string; prevReps?: string; isMain: boolean
  onWeightChange: (v: string) => void; onRepsChange: (v: string) => void; onToggle: () => void
}) {
  return (
      ...
      <div className="flex-1">
        <div className="flex items-center bg-bg border border-border rounded-xl overflow-hidden">
          <button onClick={() => onWeightChange(String(Math.max(0, parseFloat(weight || '0') - 1.25)))}
            className="px-2 py-2.5 text-muted hover:text-ink active:bg-bg-alt transition-colors">
...
export function ActiveWorkout({ plan, weekIdx, dayIdx, logs, onLogsChange, onFinish }: Props) {
    ...
    const updated: TrainingLogs = {
      ...
      [key]: {
        ...
        sets: {
          ...(localLogs[key]?.sets || {}),
          [si]: {
            weight: field === 'weight' ? value : (localLogs[key]?.sets?.[si]?.weight || ''),
            reps: field === 'reps' ? value : (localLogs[key]?.sets?.[si]?.reps || '')
          }
        },
  ...
  const isNewRecord = (ri: number) => {
    ...
    const currentBest = Math.max(0, ...sets.map(s => parseFloat(s.weight || '0')))
    ...
    <div className="fixed inset-0 z-40 bg-bg flex flex-col">
      ...
      <div className="bg-card border-b border-border flex-shrink-0">
        <div className="flex items-center gap-3 px-4 py-3">
          <button onClick={() => setShowFinish(true)} className="p-2 rounded-xl hover:bg-bg-alt text-muted hover:text-ink transition-colors">
          ...
          <button onClick={() => setShowFinish(true)} className="px-3 py-2 bg-ok text-white rounded-xl text-xs font-bold hover:opacity-90">
            Terminar
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {day.exercises.map((ex, ri) => {
          ...
          return (
            <div key={ri} className={`border-b border-border ${isDone ? 'bg-ok/3' : ''}`}>
              <div className="flex items-center gap-3 px-4 py-3.5 cursor-pointer active:bg-bg-alt" onClick={() => setOpenEx(isOpen ? -1 : ri)}>
              ...
              {isOpen && (
                <div className="pb-2">
                  ...
                  <div className="flex items-center gap-2 px-4 pb-1">
                    ...
                    <div className="flex-1 text-center"><p className="text-[9px] uppercase tracking-wider text-muted font-bold">Kg</p></div>
                    <div className="flex-1 text-center"><p className="text-[9px] uppercase tracking-wider text-muted font-bold">Reps</p></div>
                  ...
                  {Array.from({ length: numSets }, (_, si) => {
                    ...
                    return (
                      <SetRow key={si} setNum={si + 1}
                        weight={s.weight || ''} reps={s.reps || String(numReps)}
                        done={!!(isDone)} prevWeight={prev?.weight} prevReps={prev?.reps}
                        isMain={ex.isMain}
      ...
      {showFinish && (
        <div className="fixed inset-0 z-50 bg-ink/80 backdrop-blur-sm flex items-end">
          <div className="w-full bg-card rounded-t-3xl p-6 space-y-4">
            ...
            <button onClick={onFinish} className="w-full py-4 bg-ok text-white rounded-2xl font-bold text-base hover:opacity-90 active:scale-[0.98] transition-all">
              ✓ Terminar y guardar
            </button>
            <button onClick={() => setShowFinish(false)} className="w-full py-3 border border-border rounded-2xl text-sm font-medium text-muted hover:bg-bg-alt transition-colors">
              Seguir entrenando
            </button>
          </div>
        </div>
