  ...
  if (view === 'landing') return (
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <section className="flex flex-col items-center justify-center px-6 pt-20 pb-16 text-center relative overflow-hidden">
        ...
        <p className="relative text-xs font-bold uppercase tracking-[0.25em] text-accent mb-8">
        ...
        <h1 className="relative text-6xl sm:text-7xl lg:text-8xl font-serif font-bold leading-[0.92] mb-8 max-w-4xl">
          Un panel<br />
          <span className="text-accent italic">único</span><br />
          por cliente
        </h1>
        <p className="relative text-lg text-muted max-w-lg mx-auto mb-12 leading-relaxed">
          ...
          — plan, vídeos, progreso y comunicación directa contigo.
        </p>
        <div className="relative flex flex-col sm:flex-row gap-3 justify-center mb-16">
          {onDemo && (
            <button onClick={onDemo}
              className="flex items-center justify-center gap-2 px-8 py-4 bg-ink text-white rounded-xl text-sm font-bold hover:opacity-90 transition-opacity shadow-lg">
              Ver demo en vivo <ArrowRight className="w-4 h-4" />
            </button>
          )}
          <button onClick={() => setView('register')}
            className="flex items-center justify-center gap-2 px-8 py-4 border border-border rounded-xl text-sm font-semibold text-muted hover:border-ink hover:text-ink transition-all">
            Solicitar acceso
          </button>
        </div>
      ...
      <section className="max-w-5xl mx-auto px-6 py-20 w-full">
        ...
        <h2 className="text-4xl sm:text-5xl font-serif font-bold text-center mb-16 max-w-2xl mx-auto leading-tight">
          Todo lo que necesitas para gestionar a tus alumnos
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {FEATURES.map(({ icon, title, desc }) => (
            <div key={title}
              className="bg-card border border-border rounded-2xl p-8 hover:border-accent/40 hover:shadow-sm transition-all group">
      ...
      <section className="bg-ink text-white">
        <div className="max-w-5xl mx-auto px-6 py-24 text-center">
          ...
          <p className="text-white/50 max-w-md mx-auto mb-10 text-sm leading-relaxed">
            Sin apps que instalar, sin complicaciones. Tu cliente accede con un enlace y tú controlas todo desde el panel.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {onDemo && (
              <button onClick={onDemo}
                className="flex items-center justify-center gap-2 px-8 py-4 bg-white text-ink rounded-xl text-sm font-bold hover:opacity-90 transition-opacity">
                Ver demo en vivo <ArrowRight className="w-4 h-4" />
              </button>
            )}
            <button onClick={() => setView('register')}
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 bg-ink text-white p-16">
        ...
        <div>
          <h2 className="text-5xl font-serif font-bold leading-tight mb-6">
            Tu negocio de<br />
            <span className="text-accent italic">entrenamiento</span><br />
            en un solo lugar
          </h2>
          <p className="text-white/50 text-sm leading-relaxed max-w-sm">
            Planes personalizados, seguimiento del progreso y comunicación directa con tus clientes.
          </p>
        </div>
        <button onClick={() => setView('landing')} className="text-white/40 text-xs hover:text-white/70 transition-colors text-left">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <h2 className="text-2xl font-serif font-bold mb-2">
            {view === 'login' ? 'Bienvenido de nuevo' : 'Solicitar acceso'}
          </h2>
          <p className="text-muted text-sm mb-8">
            {view === 'login' ? 'Entra a tu panel de entrenador.' : 'Crea tu cuenta y empieza a gestionar clientes.'}
          </p>
            ...
            {view === 'register' && (
              <div>
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)}
                  ...
                  className="w-full px-4 py-3 bg-card border border-border rounded-xl outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent text-sm transition-all"
                />
              </div>
            )}
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} value={password}
                  ...
                  onKeyDown={e => e.key === 'Enter' && (view === 'login' ? handleLogin() : handleRegister())}
                  placeholder="••••••••"
                  className="w-full px-4 py-3 pr-10 bg-card border border-border rounded-xl outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent text-sm transition-all"
                />
          ...
          <button
            className="w-full mt-6 py-3.5 bg-ink text-white rounded-xl text-sm font-bold hover:opacity-90 disabled:opacity-50 transition-opacity flex items-center justify-center gap-2"
