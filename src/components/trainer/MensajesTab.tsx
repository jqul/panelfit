...
import { Plus, Trash2, Send, Calendar, Edit2, X, Check } from 'lucide-react'

interface Encuesta { id: string; nombre: string; preguntas: string[] }
interface MensajePlantilla { id: string; tipo: 'recordatorio' | 'bienvenida' | 'checkin' | 'personalizado'; nombre: string; texto: string }
interface Programacion { id?: string; clientId: string; clientName: string; tipo: string; mensaje: string; encuestaId?: string; fechaEnvio: string; enviado: boolean }
interface Props { userProfile: UserProfile; clients: ClientData[] }
...
export function MensajesTab({ userProfile, clients }: Props) {
  ...
  const [nuevoProg, setNuevoProg] = useState<Partial<Programacion>>({ tipo: 'recordatorio', fechaEnvio: new Date().toISOString().split('T')[0], enviado: false })

  useEffect(() => { loadProgramacion() }, [])

  const loadProgramacion = async () => {
    const { data } = await supabase.from('programacion_mensajes').select('*').order('fechaEnvio', { ascending: true })
    if (data) setProgramacion(data.map((d: any) => ({ id: d.id, clientId: d.clientId, clientName: clients.find(c => c.id === d.clientId)?.name || '—', tipo: d.tipo, mensaje: d.mensaje, encuestaId: d.encuestaId, fechaEnvio: d.fechaEnvio, enviado: d.enviado })))
  }
  ...
  const fillVars = (texto: string, client: ClientData) => {
    const url = `${window.location.origin}?c=${client.token}`
    return texto.replace(/{nombre}/g, client.name).replace(/{dias}/g, '0').replace(/{enlace}/g, url).replace(/{encuesta}/g, `${url}&encuesta=1`)
  }

  const enviarWhatsApp = (prog: Programacion) => {
    const client = clients.find(c => c.id === prog.clientId)
    if (!client) return
    window.open(`https://wa.me/?text=${encodeURIComponent(fillVars(prog.mensaje, client))}`, '_blank')
    if (prog.id) supabase.from('programacion_mensajes').update({ enviado: true }).eq('id', prog.id).then(() => loadProgramacion())
    toast('WhatsApp abierto ✓', 'ok')
  }

  const guardarProgramacion = async () => {
    if (!nuevoProg.clientId || !nuevoProg.mensaje || !nuevoProg.fechaEnvio) { toast('Completa todos los campos', 'warn'); return }
    const { error } = await supabase.from('programacion_mensajes').insert({ trainerId: userProfile.uid, clientId: nuevoProg.clientId, tipo: nuevoProg.tipo, mensaje: nuevoProg.mensaje, encuestaId: nuevoProg.encuestaId, fechaEnvio: nuevoProg.fechaEnvio, enviado: false, createdAt: Date.now() })
    if (error) { toast('Error al guardar', 'warn'); return }
    toast('Programado ✓', 'ok'); setShowNuevoProg(false)
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex gap-1 bg-bg p-1 rounded-xl border border-border w-fit">
        {[{ id: 'plantillas', label: '✍️ Plantillas' }, { id: 'encuestas', label: '📋 Encuestas' }, { id: 'programacion', label: '📅 Programación' }].map(t => (
      ...
      {tab === 'plantillas' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-xs text-muted flex gap-1 flex-wrap">Variables: {VARS_AYUDA.map(v => <code key={v} className="bg-bg-alt px-1 py-0.5 rounded text-[10px]">{v}</code>)}</p>
          ...
          {editandoPlantilla && (
            <div className="bg-card border border-accent/20 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-semibold">{nuevaPlantilla ? 'Nueva plantilla' : 'Editar'}</h4>
                <button onClick={() => { setEditandoPlantilla(null); setNuevaPlantilla(false) }}><X className="w-4 h-4 text-muted" /></button>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-muted mb-1">Nombre</label>
                  <input type="text" value={editandoPlantilla.nombre} onChange={e => setEditandoPlantilla({ ...editandoPlantilla, nombre: e.target.value })}
                ...
                <div>
                  ...
                  <select value={editandoPlantilla.tipo} onChange={e => setEditandoPlantilla({ ...editandoPlantilla, tipo: e.target.value as any })}
              ...
              <div>
                ...
                <textarea rows={4} value={editandoPlantilla.texto} onChange={e => setEditandoPlantilla({ ...editandoPlantilla, texto: e.target.value })}
                  placeholder="Hola {nombre} 👋..." className="w-full px-3 py-2 bg-bg border border-border rounded-xl text-sm outline-none resize-none" />
                <div className="flex gap-1 mt-1 flex-wrap">
                  {VARS_AYUDA.map(v => (
                    <button key={v} onClick={() => setEditandoPlantilla({ ...editandoPlantilla, texto: editandoPlantilla.texto + v })}
                      className="text-[10px] bg-bg-alt border border-border px-2 py-0.5 rounded text-muted hover:border-accent">{v}</button>
                  ))}
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => { setEditandoPlantilla(null); setNuevaPlantilla(false) }} className="flex-1 py-2 border border-border rounded-xl text-sm text-muted">Cancelar</button>
                <button onClick={() => {
                  const existe = plantillas.find(p => p.id === editandoPlantilla.id)
                  const updated = existe ? plantillas.map(p => p.id === editandoPlantilla.id ? editandoPlantilla : p) : [...plantillas, editandoPlantilla]
                  savePlantillas(updated); toast('Plantilla guardada ✓', 'ok'); setEditandoPlantilla(null); setNuevaPlantilla(false)
                }} className="flex-1 py-2 bg-ink text-white rounded-xl text-sm font-semibold">Guardar</button>
              </div>
            </div>
          )}
          <div className="space-y-3">
            {(['recordatorio', 'bienvenida', 'checkin', 'personalizado'] as const).map(tipo => {
              const grupo = plantillas.filter(p => p.tipo === tipo)
              if (!grupo.length) return null
              const labels = { recordatorio: '⚠️ Recordatorio', bienvenida: '👋 Bienvenida', checkin: '📋 Check-in', personalizado: '✍️ Personalizado' }
              return (
                <div key={tipo}>
                      ...
                      <div key={p.id} className="bg-card border border-border rounded-xl p-4">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="flex gap-1 flex-shrink-0">
                            <button onClick={() => setEditandoPlantilla(p)} className="p-1.5 text-muted hover:text-accent"><Edit2 className="w-3.5 h-3.5" /></button>
                            {!PLANTILLAS_DEFAULT.find(d => d.id === p.id) && (
                              <button onClick={() => { savePlantillas(plantillas.filter(x => x.id !== p.id)); toast('Eliminada', 'ok') }} className="p-1.5 text-muted hover:text-warn"><Trash2 className="w-3.5 h-3.5" /></button>
                            )}
                          </div>
                        </div>
      ...
      {tab === 'encuestas' && (
        <div className="space-y-4">
          ...
          {editandoEncuesta && (
            <div className="bg-card border border-accent/20 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-semibold">{nuevaEncuesta ? 'Nueva encuesta' : 'Editar'}</h4>
                <button onClick={() => { setEditandoEncuesta(null); setNuevaEncuesta(false) }}><X className="w-4 h-4 text-muted" /></button>
              </div>
              <div>
                <label className="block text-xs font-semibold text-muted mb-1">Nombre</label>
                <input type="text" value={editandoEncuesta.nombre} onChange={e => setEditandoEncuesta({ ...editandoEncuesta, nombre: e.target.value })}
                  placeholder="Ej: Check-in semanal" className="w-full px-3 py-2 bg-bg border border-border rounded-lg text-sm outline-none" />
              </div>
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-muted">Preguntas</label>
                {editandoEncuesta.preguntas.map((p, i) => (
                  <div key={i} className="flex gap-2">
                    <input type="text" value={p} onChange={e => { const ps = [...editandoEncuesta.preguntas]; ps[i] = e.target.value; setEditandoEncuesta({ ...editandoEncuesta, preguntas: ps }) }}
                      placeholder={`Pregunta ${i + 1}`} className="flex-1 px-3 py-2 bg-bg border border-border rounded-lg text-sm outline-none" />
                    {editandoEncuesta.preguntas.length > 1 && (
                      <button onClick={() => setEditandoEncuesta({ ...editandoEncuesta, preguntas: editandoEncuesta.preguntas.filter((_, idx) => idx !== i) })} className="p-2 text-muted hover:text-warn"><X className="w-4 h-4" /></button>
                    )}
                  </div>
                ))}
                <button onClick={() => setEditandoEncuesta({ ...editandoEncuesta, preguntas: [...editandoEncuesta.preguntas, ''] })} className="text-sm text-accent hover:underline">+ Añadir pregunta</button>
              </div>
              <div className="flex gap-2">
                <button onClick={() => { setEditandoEncuesta(null); setNuevaEncuesta(false) }} className="flex-1 py-2 border border-border rounded-xl text-sm text-muted">Cancelar</button>
                <button onClick={() => {
                  const existe = encuestas.find(e => e.id === editandoEncuesta.id)
                  const updated = existe ? encuestas.map(e => e.id === editandoEncuesta.id ? editandoEncuesta : e) : [...encuestas, editandoEncuesta]
                  saveEncuestas(updated); toast('Encuesta guardada ✓', 'ok'); setEditandoEncuesta(null); setNuevaEncuesta(false)
                }} className="flex-1 py-2 bg-ink text-white rounded-xl text-sm font-semibold">Guardar</button>
              </div>
            </div>
          )}
                ...
                <div key={enc.id} className="bg-card border border-border rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => setEditandoEncuesta(enc)} className="p-1.5 text-muted hover:text-accent"><Edit2 className="w-3.5 h-3.5" /></button>
                      <button onClick={() => { saveEncuestas(encuestas.filter(e => e.id !== enc.id)); toast('Eliminada', 'ok') }} className="p-1.5 text-muted hover:text-warn"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                  <p className="text-xs text-muted">{enc.preguntas.length} preguntas</p>
                  {enc.preguntas.slice(0, 3).map((p, i) => <p key={i} className="text-xs text-muted">• {p}</p>)}
      ...
      {tab === 'programacion' && (
        <div className="space-y-4">
          <button onClick={() => setShowNuevoProg(true)} className="flex items-center gap-2 px-3 py-2 bg-ink text-white rounded-lg text-sm font-semibold hover:opacity-90">
            ...
            <div className="bg-card border border-accent/20 rounded-2xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={nuevoProg.clientId || ''} onChange={e => setNuevoProg({ ...nuevoProg, clientId: e.target.value })}
                    className="w-full px-3 py-2 bg-bg border border-border rounded-lg text-sm outline-none">
                ...
                <div>
                  ...
                  <input type="date" value={nuevoProg.fechaEnvio} onChange={e => setNuevoProg({ ...nuevoProg, fechaEnvio: e.target.value })}
                    className="w-full px-3 py-2 bg-bg border border-border rounded-lg text-sm outline-none" />
                </div>
              </div>
              <div>
              ...
              {nuevoProg.tipo === 'encuesta' && encuestas.length > 0 && (
                <div>
                  ...
                  <select value={nuevoProg.encuestaId || ''} onChange={e => setNuevoProg({ ...nuevoProg, encuestaId: e.target.value })}
                    className="w-full px-3 py-2 bg-bg border border-border rounded-lg text-sm outline-none">
              ...
              <div>
                <label className="block text-xs font-semibold text-muted mb-1">Mensaje</label>
                <select onChange={e => { const p = plantillas.find(p => p.id === e.target.value); if (p) setNuevoProg({ ...nuevoProg, mensaje: p.texto }) }}
                  className="w-full px-3 py-2 bg-bg border border-border rounded-lg text-xs outline-none text-muted mb-2">
                  <option value="">Usar plantilla...</option>
                  {plantillas.map(p => <option key={p.id} value={p.id}>{p.nombre}</option>)}
                </select>
                <textarea rows={4} value={nuevoProg.mensaje || ''} onChange={e => setNuevoProg({ ...nuevoProg, mensaje: e.target.value })}
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowNuevoProg(false)} className="flex-1 py-2.5 border border-border rounded-xl text-sm text-muted">Cancelar</button>
                <button onClick={guardarProgramacion} className="flex-1 py-2.5 bg-ink text-white rounded-xl text-sm font-semibold">
                ...
                <div className="bg-warn/5 border border-warn/20 rounded-2xl p-4 space-y-2">
                  ...
                  {pendientesHoy.map(p => (
                    <div key={p.id} className="flex items-center gap-3 bg-card border border-border rounded-xl p-3">
                      ...
                      <button onClick={() => enviarWhatsApp(p)} className="flex items-center gap-1.5 px-3 py-2 bg-[#25D366] text-white rounded-lg text-xs font-bold flex-shrink-0">
              ...
              <div className="bg-card border border-border rounded-2xl divide-y divide-border overflow-hidden">
                {programacion.map(p => (
                  <div key={p.id} className={`flex items-center gap-3 px-4 py-3 ${p.enviado ? 'opacity-50' : ''}`}>
                    ...
                    {!p.enviado ? (
                      <button onClick={() => enviarWhatsApp(p)} className="p-2 text-[#25D366] hover:bg-[#25D366]/10 rounded-lg flex-shrink-0"><Send className="w-4 h-4" /></button>
