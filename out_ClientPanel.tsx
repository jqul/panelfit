...
export function ClientPanel({ client, userProfile, allClients, onClose }: Props) {
  ...
  const [activeSession, setActiveSession] = useState<{ day: any; dayKey: string } | null>(null)
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {loading ? (
              ...
              <>
                ...
                {/* Vista previa del cliente — el entrenador ve exactamente lo que verá el cliente */}
                {activeTab === 'vista' && plan && plan.weeks.length > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 p-4 bg-accent/5 border border-accent/20 rounded-xl">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-accent">Vista previa del cliente</p>
                        <p className="text-xs text-muted mt-0.5">Esto es exactamente lo que ve el cliente en su enlace. Puedes probar la sesión completa.</p>
                      </div>
                    </div>
                    {activeSession ? (
                      <TrainingSession
